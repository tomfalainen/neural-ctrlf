...
function net_utils.compute_field_centers(net, end_idx)
  ...
  for i = 1, end_idx do
    ...
    if t == 'nn.SpatialConvolution' or
      ...
      local same_conv = same_x and same_y
      if layer.dW == 2 and layer.dH == 2 then
        x0 = x0 + sx / 2
        y0 = y0 + sy / 2
        sx = 2 * sx
        sy = 2 * sy
    ...
    elseif t == 'nn.ReLU' or t == 'cudnn.ReLU' or t == 'nn.SpatialBatchNormalization' then
      -- Do nothing
    elseif t == 'nn.SpatialMaxPooling' or
      ...
      if layer.kW ~= 2 or layer.kH ~= 2 or
         ...
         if layer.kW ~= 3 or layer.kH ~= 3 then
