...
require 'ctrlfnet.modules.ReshapeBoxFeatures'
require 'ctrlfnet.modules.ApplyBoxTransform'
require 'ctrlfnet.modules.InvertBoxTransform'
...
function layer:_forward_train(input)
  ...
  local gt_boxes, gt_embeddings = self.gt_boxes, self.gt_embeddings
  ...
  assert(gt_embeddings:dim() == 3 and gt_embeddings:size(1) == N and gt_embeddings:size(2) == B1,
  ...
  self:timeit('sampler:forward', function()
    ...
    local sampler_out = self.nets.box_sampler_helper:forward{
                          self.rpn_out, {gt_boxes, gt_embeddings}}
  ...
  if self.dump_vars then
    ...
    vars.positive_indices = self.nets.box_sampler_helper.pos_target_idx
    self.stats.vars = vars
  end
...
function layer:updateGradInput(input, gradOutput)
  ...
  assert(self.gt_boxes and self.gt_embeddings and not self._called_backward_gt,
  ...
  local gt_embeddings = self.gt_embeddings
...
function build_rpn(opt)
  ...
  local conv_layer = cudnn.SpatialConvolution(opt.input_dim, opt.rpn_num_filters, opt.rpn_filter_size, opt.rpn_filter_size, 1, 1, pad, pad)
  ...
  local box_conv_layer = cudnn.SpatialConvolution(opt.rpn_num_filters, 4 * num_anchors, 1, 1, 1, 1, 0, 0)
  ...
  local rpn_conv_layer = cudnn.SpatialConvolution(opt.rpn_num_filters, 2 * num_anchors, 1, 1, 1, 1, 0, 0)
