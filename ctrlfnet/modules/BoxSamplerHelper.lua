require 'ctrlfnet.modules.BoxSampler'
...
function helper:set_labels(labels)
...
function helper:updateOutput(input)
  ...
  local n = self.pos_target_idx:size(1)
  self.y = torch.ones(n):type(self.pos_target_idx:type())
  local frac = 5 -- The fraction of how many positive pairs are kept 3 = 66% positive
  local z = torch.randperm(n):lt(n / frac) --:type(self.pos_target_idx:type())
  self.y[z] = -1
  local p = z:clone():double() --:type(self.pos_target_idx:type())
  ...
  local modified_pos_target_idx = self.pos_target_idx:clone()
  modified_pos_target_idx[z] = torch.multinomial(p, z:sum(), true):type(self.pos_target_idx:type())
