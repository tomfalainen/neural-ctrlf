require 'ctrlfnet.modules.BoxIoU'
...
function BoxSampler:set_labels(labels)
  self.labels = labels
end
...
function BoxSampler:sample(input_idx)
...
function BoxSampler:update_histogram(pos_target_idx)
...
function BoxSampler:updateOutput(input)
  ...
  local target_max_iou, target_idx = ious:max(2) -- N x 1 x B2
  ...
  if self.biased_sampling then pos_p = self:sample(input_idx) end
  ...
  if self.biased_sampling then self:update_histogram(pos_target_idx) end
