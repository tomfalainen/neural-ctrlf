...
require 'ctrlfnet.LocalizationLayer'
require 'ctrlfnet.modules.BoxRegressionCriterion'
require 'ctrlfnet.modules.BilinearRoiPooling'
require 'ctrlfnet.modules.ApplyBoxTransform'
require 'ctrlfnet.modules.LogisticCriterion'
require 'ctrlfnet.modules.PosSlicer'

local box_utils = require 'ctrlfnet.box_utils'
local utils = require 'ctrlfnet.utils'
local net_utils = require 'ctrlfnet.net_utils'
local createModel = require 'misc.presnet'

local WordSpottingModel, parent = torch.class('WordSpottingModel', 'nn.Module')

function WordSpottingModel:__init(opt)
  local net_utils = require 'ctrlfnet.net_utils'
  opt = opt or {}  
  ...
  opt.cosine_margin = utils.getopt(opt, 'cosine_margin', 0.1)
  ...
  opt.finetune_full = false

  opt.fc_size = 4096
  opt.descriptor_size = 512
  if opt.embedding == 'dct' then
    opt.embedding_size = 108
  elseif opt.embedding == 'phoc' then
    opt.embedding_size = 540
  end
  ...
  cnn = self:_load_presnet(opt.descriptor_size)
  embedding_net = self:_build_embedding_net(opt)
  ...
  local conv_start1, conv_end1 = 1, 4 -- these will not be finetuned for efficiency
  local conv_start2, conv_end2 = 5, 6 -- these will be finetuned possibly
  local recog_start, recog_end = 7, 14 -- The rest
  opt.input_dim = 128                   -- number of feature maps as input to the localization layer
  opt.output_height, opt.output_width = 8, 20
  local fc_dim = opt.descriptor_size

  -- Now that we have the indices, actually chop up the CNN.
  opt.finetune_full = true
  if opt.finetune_full then -- a bit of a hack, but nevermind
    self.nets.conv_net1 = nn.Identity()
    self.nets.conv_net2 = net_utils.subsequence(cnn, conv_start1, conv_end2)
  else
  ...
  if opt.emb_pretrain ~= '' then
    self.nets.recog_base = cp.model.nets.recog_base
  end
  self.nets.embedding_net = embedding_net
...
function WordSpottingModel:_load_presnet(descriptor_size)
    descriptor_size = descriptor_size or 512
    local options = {}
    options.dataset = 'imagenet'
    options.depth = 34
    options.descriptor_size = descriptor_size
    local model = createModel(options)
    return model
end  

function WordSpottingModel:_build_embedding_net(opt)
...
function WordSpottingModel:_buildRecognitionNet()
  ...
  local emb_output = self.nets.embedding_net(pos_roi_codes)
...
function WordSpottingModel:training()
  parent.training(self)
  self.net:training()
end


function WordSpottingModel:evaluate()
...
--[[
Set test-time parameters for this WordSpottingModel.
...
--]]
function WordSpottingModel:setTestArgs(kwargs)
...
--[[
Convert this WordSpottingModel to a particular datatype, and convert convolutions
between cudnn and nn.
--]]
function WordSpottingModel:convert(dtype, use_cudnn)
  self:type(dtype)
...
Input:
- image: Pixel data for a single image of shape (1, 1, H, W)
...
for that image, and assume that there are P ground-truth regions. 
...
- emb_output: At training time, an array of shape (N, R) giving embeddings
  for each region.
  At test time, an array of shape (N, R) embeddings for each region.
...
- gt_labels: At training time, an array of shape (P,) giving ground-truth
...
function WordSpottingModel:updateOutput(input)
  ...
  assert(input:dim() == 4 and input:size(1) == 1 and input:size(2) == 1)
  ...
  if not self.train and self.opt.final_nms_thresh > 0 then
    ...
    local emb_output_float = self.output[5]:float()
    ...
    self.output[5] = emb_output_float:index(1, idx):typeAs(self.output[5])
  end
...
function WordSpottingModel:extractFeatures(input)
  -- Make sure the input is (1, 1, H, W)
  assert(input:dim() == 4 and input:size(1) == 1 and input:size(2) == 1)
  local H, W = input:size(3), input:size(4)
  self.nets.localization_layer:setImageSize(H, W)
  ...
  local logprobs = class_scores_float:index(1, idx)

  return boxes_xcycwh, feats, logprobs
end
...
Input: Tensor of shape (1, 1, H, W) giving pixels for an input image.
...
--]]
function WordSpottingModel:forward_test(input)
  self:evaluate()
  local output = self:forward(input)
  local final_boxes = output[4]
  local objectness_scores = output[1]
  local embeddings = output[5]

  return final_boxes, objectness_scores, embeddings
end


function WordSpottingModel:setGroundTruth(gt_boxes, gt_embeddings, gt_labels)
  self.gt_boxes = gt_boxes
  self.gt_labels = gt_labels
  self.gt_embeddings = gt_embeddings
  self._called_forward = false
  self.nets.localization_layer:setGroundTruth(gt_boxes, gt_embeddings, gt_labels)
end


function WordSpottingModel:backward(input, gradOutput)
...
- grad_cnn_params: Flattened gradients for the portion of the CNN that will
  be fine-tuned.
--]]
function WordSpottingModel:getParameters()
  ...
  -- modules[1] = identity
  -- modules[2] = recog_base
  -- modules[3] = objectness_branch
  -- modules[4] = identity
  -- modules[5] = identity
  -- modules[6] = PosSlicer
  -- modules[7] = identity
  -- modules[8] = PosSlicer
  -- modules[9] = box_reg_branch
  -- modules[10] = ApplyBoxTransform
  -- modules[11] = embedding_net

  -- Don't train some parts of the network
  -- for i = 1, 10 do
  --   if i ~= 2 and i ~= 11 then 
  --     fakenet:add(self.net:get(4).modules[i])
  --   end
  -- end
...
function WordSpottingModel:clearState()
  self.net:clearState()
...
This is a nonstandard method, but it allows the WordSpottingModel to
...
- gt_labels: 1 x B x 1 array of ground-truth labels for boxes
--]]
function WordSpottingModel:forward_backward(data)
  self:training()

  -- Run the model forward
  self:setGroundTruth(data.gt_boxes, data.embeddings, data.labels)
  ...
  local emb_output = out[5]
  local gt_boxes = out[6]
  local gt_embeddings = out[7]
  ...
  local y = self.nets.localization_layer.y:type(gt_embeddings:type()) 

  local embedding_loss = self.crits.emb_crit:forward({emb_output, gt_embeddings}, y)
  embedding_loss = embedding_loss * self.opt.embedding_weight
  local grad_emb_output, dummy = unpack(self.crits.emb_crit:backward({emb_output, gt_embeddings}, y))
  grad_emb_output:mul(self.opt.embedding_weight)
  ...
  grad_out[5] = grad_emb_output
  grad_out[6] = gt_boxes.new(#gt_boxes):zero()
  grad_out[7] = gt_embeddings.new(#gt_embeddings):zero()
