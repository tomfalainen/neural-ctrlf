...
function utils.build_loss_string(losses)
  ...
  for k,v in pairs(losses) do
    if k ~= 'total_loss' then
      x = x .. string.format('%s: %.5f, ', k, v)
    end
  end
  x = x .. string.format(' [total: %.5f]', losses.total_loss)
  return x
end
...
--Works on descriptors (L2 distance calculated in python)
function utils.map_eval(dataset, id, embedding, fold)
  os.execute('python -W ignore misc/map_eval.py ' .. fold .. ' ' .. dataset .. ' ' .. embedding)
  local result_struct = utils.read_json('tmp/' .. dataset .. '_fold' .. fold .. '_ws_results.json') 
  return result_struct
end

function utils.ArrangeByLabel(labels)
